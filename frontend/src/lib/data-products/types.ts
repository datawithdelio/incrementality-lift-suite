...
export type DatasetPreview={rows:Record<string,string>[];columns:ColumnSummary[];total_rows:number;page:number;page_size:number;total_pages:number;date_range:{column:string;minimum:string;maximum:string}|null;treatment_distribution:Record<string,number>;outcome_distribution:Record<string,number>};
