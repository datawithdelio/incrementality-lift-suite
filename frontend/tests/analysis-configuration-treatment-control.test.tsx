...
import {
  AnalysisRunApiError,
} from "../src/lib/analysis-configuration/api";
...
describe(
  "Analysis Configuration treatment and control",
  () => {
    beforeEach(() => {
      vi.clearAllMocks();
    ...
    async function moveToFilters(
      estimatorName: RegExp,
    ): Promise<void> {
      render(
        <AnalysisConfigurationClient
    ...
    it("queues the reviewed analysis once and redirects to the returned run", async () => {
      ...
      queueAnalysisRunMock.mockReturnValue(
        queuePromise,
      );
