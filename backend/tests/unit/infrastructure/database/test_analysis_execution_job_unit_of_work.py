...
def build_unit_of_work(
    ...
    return SqlAlchemyAnalysisExecutionJobUnitOfWork(
...
@pytest.mark.asyncio
async def test_enters_with_execution_job_repository() -> None:
    session = FakeAsyncSession()
...
async def test_cannot_commit_before_entering() -> None:
    ...
    with pytest.raises(
        RuntimeError,
        match=("analysis-execution-job Unit of Work must be entered before use"),
    ):
        await unit_of_work.commit()
